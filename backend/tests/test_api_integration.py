...
def mock_pool():
    ...
    pool.transaction.return_value.__aenter__ = AsyncMock()
    return pool


@pytest.fixture
def client(mock_user, mock_pool, contract_model_artifacts):
    """
    ...
    # Exercise real app middleware/routes/model; external startup is isolated.
    from ml.inference import predictor
    assert predictor.load(*contract_model_artifacts), "Contract test bundle must load"
    with patch("main.init_firebase"), patch("main.init_db_pool", new=AsyncMock()), \
            patch("main.close_db_pool", new=AsyncMock()):
...
class TestHealthEndpoint:
    ...
    def test_health_reports_readiness(self, client):
        """Uninitialized Firebase must make readiness unavailable."""
...
class TestEstimateEndpoints:
    ...
    def test_share_estimate_not_found(self, client, mock_pool):
        ...
        # Missing estimates must not become internal limiter errors.
        assert resp.status_code == 404
...
class TestAuthGuard:
    """Real auth dependency with isolated external startup."""
    ...
    def test_root_endpoint_no_auth_needed(self, client):
        from main import app
        app.dependency_overrides.clear()
