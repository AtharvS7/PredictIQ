...
from app.services.isolated_parser import parse_isolated
...
async def parse_stored_document(doc, user_id: str) -> dict:
    ...
    try:
        result = await run_in_threadpool(parse_isolated, content, doc["mime_type"])
    except ValueError:
        raise HTTPException(status_code=422, detail="Could not parse document; upload a valid PDF, DOCX or TXT file") from None
    except RuntimeError:
        raise HTTPException(status_code=503, detail="Document processing is busy; retry shortly") from None
